...
contract MultiHop {
    ...
    function performMultiHopWithRouter(
        address mkr,
        address weth,
        address elon,
        uint256 deadline
        ...
        uint256 amountIn = IERC20(mkr).balanceOf(address(this));

        IERC20(mkr).approve(router, amountIn);

        IUniswapV2Router(router).swapExactTokensForTokens(
            IERC20(mkr).balanceOf(address(this)),
            0,
            path,
            address(this),
            deadline
